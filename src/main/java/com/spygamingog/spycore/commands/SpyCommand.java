...
public class SpyCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleCreateCommand(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§cUsage: /spy create <worldname> <type> [seed] [generator]");
        ...
        if (args[1].equalsIgnoreCase("container")) {
            ...
            if (args.length < 3) {
                sender.sendMessage("§cUsage: /spy create <worldname> <normal|nether|the_end> [seed] [generator]");
                return;
            }
            String name = args[1];
            ...
            if (new java.io.File(plugin.getServer().getWorldContainer(), name).exists()) {
            ...
            // Parse optional seed and generator
            if (args.length >= 4) {
                String arg3 = args[3];
                boolean isSeed = false;

                if (arg3.equalsIgnoreCase("random")) {
                    isSeed = true;
                    // seed remains null (random)
                } else {
                    try {
                        seed = Long.parseLong(arg3);
                        isSeed = true;
                    } catch (NumberFormatException e) {
                        // Not a number, assume it's a generator
                        generator = arg3;
                    }
                }

                if (isSeed && args.length >= 5) {
                    generator = args[4];
                }
            }
            ...
            World world = plugin.getWorldManager().createWorld(null, name, env, generator, seed);
            if (world != null) {
    ...
    private void handleWorldCommand(CommandSender sender, String[] args) {
        ...
        switch (sub) {
            ...
            case "tp":
                ...
                // STRICT PATH MODE: If explicit path provided (args > 3)
                // e.g. /spy world tp maps map1 world -> args: [world, tp, maps, map1, world] -> length 5
                if (args.length > 3) {
                    ...
                    String expectedFullPath = "spycore-worlds/" + path + "/" + lastArg;

                    // Check if it matches a registered alias
                    String registeredPath = plugin.getWorldManager().getWorldAliases().get(lastArg);
                    if (registeredPath != null) {
                        if (registeredPath.equalsIgnoreCase(expectedFullPath)) {
                            target = plugin.getWorldManager().getWorld(lastArg);
                        } else {
                            sender.sendMessage("§cError: World '" + lastArg + "' exists but is located at '" + registeredPath + "', not in '" + path + "'.");
                            return;
                        }
                    } else {
                        // Not registered, check if valid world folder exists
                        java.io.File containersBase = new java.io.File(plugin.getServer().getWorldContainer(), "spycore-worlds");
                        java.io.File targetFile = new java.io.File(containersBase, path.replace("/", java.io.File.separator) + java.io.File.separator + lastArg);
                        
                        if (targetFile.exists() && targetFile.isDirectory()) {
                             // STRICT VALIDATION: Must have level.dat to be a world
                             boolean hasLevelDat = new java.io.File(targetFile, "level.dat").exists();
                             
                             if (!hasLevelDat) {
                                 sender.sendMessage("§cError: '" + lastArg + "' is not a valid world (missing level.dat).");
                                 return;
                             }
                             
                             // Safe to load
                             target = plugin.getWorldManager().loadWorld(path, lastArg);
                        } else {
                             sender.sendMessage("§cWorld not found in container '" + path + "'.");
                             return;
                        }
                    }
                } else {
                    // GLOBAL MODE (args == 3): /spy world tp <name>
                    target = plugin.getWorldManager().getWorld(lastArg);
                }
            ...
            case "create":
                ...
                if (type.equals("container")) {
                    ...
                    if (containerPath.isEmpty()) {
                        targetFile = new java.io.File(plugin.getServer().getWorldContainer(), name);
                    } else {
                        java.io.File containersBase = new java.io.File(plugin.getServer().getWorldContainer(), "spycore-worlds");
                        targetFile = new java.io.File(containersBase, containerPath.replace("/", java.io.File.separator) + java.io.File.separator + name);
                    }
                    
                    if (targetFile.exists()) {
                        sender.sendMessage("§cError: A folder named '" + name + "' already exists.");
                        return;
                    }
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
        if (args.length == 1) {
            return filter(Arrays.asList("create", "clone", "delete", "remove", "unload", "move", "world", "setspawn", "load", "container", "template", "tag", "find", "whitelist", "wake", "help"), args[0]);
        }
        ...
        if (args.length >= 2 && sub.equals("world")) {
            ...
            if (worldSub.equals("tp")) {
                ...
                List<String> suggestions = new ArrayList<>();
                // Only show containers and worlds in the current path context
                suggestions.addAll(getContainersInPath(currentPath));
            ...
            switch (sub) {
                case "whitelist":
                    if (args[1].equalsIgnoreCase("add") || args[1].equalsIgnoreCase("remove")) {
                        return filter(new ArrayList<>(plugin.getWorldManager().getWorldAliases().keySet()), args[2]);
                    }
                    break;
                case "delete":
